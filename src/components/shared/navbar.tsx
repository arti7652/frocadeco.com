...
const variantClasses = {
  ...
  'floating-bar': {
    shell: 'border-b border-transparent bg-transparent text-slate-900',
    logo: 'rounded-[1.55rem] border border-white/70 bg-white/75 shadow-[0_16px_48px_rgba(149,166,230,0.18)] backdrop-blur-xl',
    active: 'bg-[linear-gradient(135deg,#ffc6b7_0%,#c777ff_100%)] text-white',
    idle: 'text-slate-600 hover:bg-white/70 hover:text-slate-900',
    cta: 'rounded-full bg-[linear-gradient(135deg,#ffc6b7_0%,#c777ff_100%)] text-white hover:opacity-90',
    mobile: 'border-t border-white/70 bg-white/88 backdrop-blur-xl',
  },
  'utility-bar': {
...
export function Navbar() {
  ...
  const navigation = useMemo(() => SITE_CONFIG.tasks.filter((task) => task.enabled && task.key === 'image'), [])
  const infoNavigation = useMemo(
    () => [
  ...
  const secondaryRoutes = useMemo(
    () => SITE_CONFIG.tasks.filter((task) => !task.enabled && task.key !== recipe.primaryTask).slice(0, 5),
    [recipe.primaryTask]
  )
          ...
          <div className="flex shrink-0 items-center gap-2 sm:gap-3">
            ...
            {isAuthenticated ? (
              ...
              <div className="hidden items-center gap-2 md:flex">
                <Button variant="ghost" size="sm" asChild className="rounded-full px-4 transition-colors hover:bg-[#f3dcff] hover:text-[#8a5fa7]">
          ...
          <div className={palette.mobile}>
            <div className="space-y-2 px-4 py-4">
              ...
              <div className="mt-4 rounded-2xl border border-slate-200/70 bg-white/80 p-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] opacity-60">Company</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {infoNavigation.map((item) => (
                    <Link
                      ...
                      className="rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900"
                    >
                      {item.name}
                    </Link>
                  ))}
  ...
  return (
    <header className={cn('sticky top-0 z-50 w-full', style.shell)}>
      <nav className={cn('mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:px-8', isFloating ? 'h-24 pt-4' : 'h-20')}>
        <div className="flex min-w-0 flex-1 items-center gap-4 lg:gap-7">
          ...
          {isEditorial ? (
            ...
            <div className="hidden min-w-0 flex-1 items-center gap-2 xl:flex">
              <Link href="/" className={cn('flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition-colors', pathname === '/' ? style.active : style.idle)}>
        ...
        <div className="flex shrink-0 items-center gap-2 sm:gap-3">
          <Button variant="ghost" size="icon" asChild className="hidden rounded-full border border-white/70 bg-white/80 text-slate-700 hover:bg-white md:flex">
            <Link href="/search">
          ...
          {isAuthenticated ? (
            ...
            <div className="hidden items-center gap-2 md:flex">
              <Button variant="ghost" size="sm" asChild className="rounded-full border border-white/70 bg-white/80 px-4 text-slate-700 transition-colors hover:bg-[#f3dcff] hover:text-[#8a5fa7]">
                <Link href="/login">Sign In</Link>
              </Button>
              <Button size="sm" asChild className={style.cta}>
                <Link href="/register">{isEditorial ? 'Subscribe' : isUtility ? 'Post Now' : 'Create account'}</Link>
              </Button>
            </div>
          )}

          <Button variant="ghost" size="icon" className="rounded-full border border-white/70 bg-white/80 text-slate-700 hover:bg-white lg:hidden" onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
            {isMobileMenuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
          </Button>
        </div>
      </nav>
      ...
      {isMobileMenuOpen && (
        <div className={style.mobile}>
          <div className="space-y-2 px-4 py-4">
            <Link href="/search" onClick={() => setIsMobileMenuOpen(false)} className="mb-3 flex items-center gap-3 rounded-2xl border border-white/70 bg-white/82 px-4 py-3 text-sm font-semibold text-slate-700">
              <Search className="h-4 w-4" />
              Search the site
            </Link>
            ...
            <div className="mt-4 rounded-2xl border border-white/70 bg-white/78 p-4">
